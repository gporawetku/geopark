...
@can('update', $editableSchedule)
    <form method="POST" action="{{ route('schedules.update', $editableSchedule) }}" accept-charset="UTF-8">
        ...
        <div class="form-group">
            <label for="description" class="form-label">{{ __('schedule.description') }}</label>
            <textarea id="description" class="form-control{{ $errors->has('description') ? ' is-invalid' : '' }}" name="description" rows="4">{{ old('description', $editableSchedule->description) }}</textarea>
            {!! $errors->first('description', '<span class="invalid-feedback" role="alert">:message</span>') !!}
        </div>
        ...
        @can('delete', $editableSchedule)
            <a href="{{ route('schedules.index', ['action' => 'delete', 'id' => $editableSchedule->id] + Request::only('page', 'q')) }}" id="del-schedule-{{ $editableSchedule->id }}" class="btn btn-danger float-right">{{ __('app.delete') }}</a>
        @endcan
    </form>
@endcan
@endif
...
@can('delete', $editableSchedule)
    <div class="card">
        ...
        <div class="card-footer">
            <form method="POST" action="{{ route('schedules.destroy', $editableSchedule) }}" accept-charset="UTF-8" onsubmit="return confirm(&quot;{{ __('app.delete_confirm') }}&quot;)" class="del-form float-right" style="display: inline;">
